...
class BigNum(object):
    ...
    def __add__(self, other):
        ...
        Adding numbers does not normalize them. However, the result is
        normalized.
        '''
        if not isinstance(other, BigNum):
    ...
    def slow_divmod(self, other):
        ...
        digits = max(len(self.d), len(other.d))
        quotient = BigNum.zero(digits)
        remainder = BigNum(self.d)
        s = [BigNum(other.d)]
        i = 0
        # while (s[i].d[digits] <= Byte.zero()) and (s[i] <= self):
        while s[i] <= self:
            i = i + 1
        ...
        for j in range(i - 1, -1, -1):
            ...
            if remainder >= s[j]:
                ...
                quotient.d[0] |= Byte.one() # Faster version q[0] = q[0] || 1
...
if __name__ == '__main__':
    x = BigNum.h("6")
    y = BigNum.h("C")
    assert x.slow_mul(y) == x.fast_mul(y) == BigNum.h("48")

    a = BigNum.h("48")
    b = BigNum.h("C")

    # assert
    print a.slow_divmod(b)
           # == a.fast_divmod(b) == BigNum.h("6")
